...
const Checkout = async (req, res, next) => {
    ...
    if(req.session.user) {
        ...
        db.connectDB()
        .then((connection)=>{
            connection.query(sql, req.session.user.email, (err, result) => {
                ...
                connection.query(sql2, [create_date, address, restaurant_user_id], (err, result) => {
                    if (err) throw err;
                    let sql3 = 'SELECT * FROM order_unit WHERE create_date = ?;';
                    connection.query(sql3, [create_date], (err, result) => {
                        if(err) console.log(err);
...
const showPayment = (req, res, next) => {
    // const cart = req.session.cart;
    // console.log(cart);
    res.render('payment');
};

const showCheck = (req, res, next) => {
    const cart = req.session.cart;
    console.log(cart);
